...
export default function LavanderiaApp() {
  ...
  const getClave = () => { try { return localStorage.getItem("adminClave") || "9621"; } catch { return "9621"; } };
  ...
  const resetOrderCounter = async () => {
    ...
    if (pwd !== getClave()) { if (pwd !== null) alert("❌ Clave incorrecta"); return; }
  ...
  const deleteEmployee = async (id) => {
    const pwd = prompt("Contraseña para eliminar:"); if (pwd !== getClave()) { if (pwd !== null) alert("❌ Contraseña incorrecta"); return; }
  ...
  const PayMethod = ({ m }) => { const map = { nequi: ["📱 Nequi","#C792EA","rgba(199,146,234,0.15)"], daviplata: ["💜 Daviplata","#667EEA","rgba(102,126,234,0.15)"], efectivo: ["💵 Efectivo","#66BB6A","rgba(102,187,106,0.15)"] }; const [l,c,b] = map[m]||map.efectivo; return <span style={{ fontSize:12,background:b,color:c,padding:"3px 10px",borderRadius:20 }}>{l}</span>; };
                ...
                {showCambiarClave && (
                  <div style={{ background: "#0D1117", borderRadius: 12, padding: 20, marginBottom: 20, border: "1px solid rgba(255,213,79,0.3)" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <button onClick={() => {
                        if (claveActual !== getClave()) { alert("❌ La clave actual es incorrecta"); return; }
